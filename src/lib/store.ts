...
export async function claimVoucher(
  ...
  const { error: pointsErr } = await supabase
    .from("users")
    .update({ points: user.points - voucher.points_cost })
    .eq("id", userId);
...
export async function getUserRedemptions(
  ...
  const voucherIds = [...new Set(data.map((r) => r.voucher_id))];
  ...
  return data.map((r) => ({ ...r, voucher: voucherMap.get(r.voucher_id) }));
}

export async function verifyRedemption(
  qrCode: string,
  staffId: string,
): Promise<Redemption & { user: User; voucher: Voucher }> {
  ...
  if (error || !redemption) {
    console.error("Supabase error verifyRedemption:", error);
    throw new Error(error?.message || "Invalid QR code");
  }
  if (redemption.status === "verified")
    throw new Error("This voucher has already been verified");
...
export async function getRedemptionHistory(): Promise<Redemption[]> {
  ...
  return data.map((r) => ({
    ...
    voucher: voucherMap.get(r.voucher_id),
  }));
}
